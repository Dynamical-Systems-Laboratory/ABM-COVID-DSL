...
bool check_isolation(const Agent&, const std::vector<School>&,
				const std::vector<Hospital>&, bool check_in_isolation = true);
		...
		for (auto& agent : agents){
			...
			if (agent.infected() == false){
				...
				if (agent.tested()){
					...
					if (check_isolation(agent, schools, hospitals, false)){
		...
		for (auto& agent : agents){
			...
			if (agent.infected() == false){
				...
				if (agent.exposed()){
					...
					// Testing 
					if (!check_testing_transitions(agent, households, schools, 
							hospitals, state_changes, n_tested_exposed, n_tested_sy, n_waiting_for_res, n_positive, 
							n_false_neg, n_tr_hsp, n_hsp_icu, n_ih, time, dt)){
					...
					if (agent.tested_covid_positive()){
						if (!check_treatment_setup(agent, households, schools, hospitals, n_tr_hsp, n_hsp_icu, n_ih, time, dt)){
							return false;
						}
					}
...
bool check_isolation(const Agent& agent, const std::vector<School>& schools,
				const std::vector<Hospital>& hospitals, bool check_in_isolation)
{
				...
				int& n_positive, int& n_false_neg, int& n_hsp, int& n_hsp_icu, int& n_ih, double time, double dt)
{
	...
	if (state_changes.at(3)){
		...
		if (agent.exposed()){
			...
			if (!check_treatment_setup(agent, households, schools, hospitals, n_hsp, n_hsp_icu, n_ih, time, dt)){
				return false;
			}
		}
		// Correct flag
				...
				int& n_hsp, int& n_hsp_icu, int& n_ih, double time, double dt)
{
	if (agent.hospitalized()){
		...
		if (!check_hospitalized_isolation(agent, households, schools, hospitals)){
